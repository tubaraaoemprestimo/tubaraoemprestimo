...
export const FinancePage: React.FC = () => {
    ...
    const handleRunCollections = async () => {
        if (!confirm('Disparar cobranças automáticas para todas as parcelas vencidas e a vencer (regras configuradas)?\nIsso enviará WhatsApp, Email e Push.')) return;

        setRunningCollections(true);
        try {
            const result = await autoNotificationService.triggerManualCollections();
            if (result.success) {
                addToast(`Cobranças disparadas! Sucesso: ${result.results?.collections?.sent || 0}`, 'success');
            } else {
                throw new Error(result.error);
            }
        } catch (error: any) {
            console.error(error);
            addToast(`Erro ao disparar cobranças: ${error.message}`, 'error');
        } finally {
    ...
    return (
        <div className="p-4 md:p-8 bg-black min-h-screen text-white pb-32">
            ...
            {/* Actions Bar */}
            <div className="flex flex-wrap justify-between items-center mb-8 gap-4 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                <div className="flex gap-2">
                    <button
                        onClick={() => setPeriod('month')}
                        className={`px-4 py-2 rounded-lg text-sm ${period === 'month' ? 'bg-[#D4AF37] text-black font-bold' : 'bg-zinc-800 text-zinc-400'}`}
                    >
                        Este Mês
                    </button>
                    <button
                        onClick={() => setPeriod('year')}
                        className={`px-4 py-2 rounded-lg text-sm ${period === 'year' ? 'bg-[#D4AF37] text-black font-bold' : 'bg-zinc-800 text-zinc-400'}`}
                    >
                        Este Ano
                    </button>
                </div>
                ...
                <div className="flex gap-2">
                    <Button
                        ...
                        isLoading={runningCollections}
                        className="bg-red-600 hover:bg-red-700 text-white"
                    >
                        {runningCollections ? <Loader2 size={16} className="animate-spin mr-2" /> : <Zap size={16} className="mr-2" />}
                        Disparar Cobranças
                    </Button>
                </div>
            </div>
