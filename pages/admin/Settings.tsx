...
export const Settings: React.FC = () => {
  ...
  const handleTestEmail = async () => {
    if (!testEmail) {
      addToast('Digite um email para teste', 'warning');
      return;
    }
    ...
    if (success) {
      ...
      addToast('Falha ao enviar. Verifique Logs e Variáveis no Supabase.', 'error');
    }
  };
  ...
  const renderIntegrationTab = () => (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="mt-8 border-t border-zinc-800 pt-8">
        <h2 className="text-xl font-bold flex items-center gap-2 mb-2">
          <MessageSquare size={20} className="text-blue-500" /> Email Transacional (Resend)
        </h2>
        <p className="text-zinc-400 text-sm mb-6">Teste o envio de emails do domínio tubaraoemprestimo.com.br.</p>

        <div className="bg-black p-6 rounded-xl border border-zinc-800">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 w-full">
              <label className="text-xs text-zinc-500 uppercase font-bold mb-1 block">Email de Destino para Teste</label>
              <input
                type="email"
          ...
          <p className="text-xs text-zinc-600 mt-4">
            API Key configurada via código (Modo Dev). Remova antes de ir para produção real.
          </p>
        </div>
      </div>
    </div>
