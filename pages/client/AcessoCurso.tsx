  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-center">
      ...
      <div className="relative z-10 max-w-md mx-auto">
        <div className="w-24 h-24 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center mx-auto mb-6 shadow-2xl">
        ...
        <button onClick={onBack} className="text-slate-500 text-sm hover:text-slate-300 transition-colors">
          ← Voltar ao Dashboard
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center gap-6">
      ...
      <div className="text-center">
        ...
        <p className="text-slate-500 text-sm mt-1">Preparando o Método Tubarão…</p>
      </div>
    </div>
  );
}
...
function GlobalProgressBar({ pct, completed, total }: { pct: number; completed: number; total: number }) {
  ...
  return (
    <div className="bg-slate-900/80 backdrop-blur border-b border-slate-800 px-4 py-3">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400">{completed}/{total} aulas</span>
            <span className="text-sm font-black text-white">{pct}%</span>
          </div>
        </div>
        <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-yellow-600 via-yellow-500 to-yellow-400 rounded-full transition-all duration-700 ease-out relative"
        ...
        {pct > 0 && (
          <p className="text-[10px] text-slate-500 mt-1">{getMotivationalText()}</p>
        )}
      </div>
    </div>
  );
    ...
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-all group relative
        ${isActive
          ...
          : 'hover:bg-slate-800/60 border-l-[3px] border-transparent'
        }`}
    >
      {/* Status icon */}
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-xs font-semibold leading-snug line-clamp-2
          ${isActive ? 'text-yellow-400' : isCompleted ? 'text-slate-400' : 'text-slate-300 group-hover:text-white'}`}>
          {lesson.title}
        </p>
        {lesson.duration && (
          <div className="flex items-center gap-1 mt-0.5">
            <Clock size={9} className="text-slate-600" />
            <p className="text-[10px] text-slate-600">{formatDuration(lesson.duration)}</p>
          </div>
        )}
      </div>
...
function ModuleAccordion({
  ...
  return (
    <div className="border-b border-slate-800/60 last:border-0">
      <button
        onClick={() => setOpen(o => !o)}
        className={`w-full flex items-center gap-3 px-4 py-4 hover:bg-slate-800/40 transition-all group`}
      >
        {/* Module number */}
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-xs font-black transition-colors
          ${allDone ? 'bg-yellow-500 text-black' : isActiveModule ? 'bg-yellow-500/20 text-yellow-500 border border-yellow-500/40' : 'bg-slate-800 text-slate-500'}`}>
          {allDone ? <CheckCircle2 size={14} /> : moduleIndex + 1}
        </div>

        {/* Title + progress */}
        <div className="flex-1 text-left">
          <p className={`text-xs font-bold leading-snug ${isActiveModule ? 'text-white' : 'text-slate-300 group-hover:text-white'} transition-colors`}>
            {module.title}
          </p>
        ...
        {open
          ? <ChevronDown size={14} className="text-slate-500 shrink-0 transition-transform" />
          : <ChevronRight size={14} className="text-slate-600 shrink-0 transition-transform" />}
      </button>
      ...
      `}>
        ...
        <div className="p-4 border-b border-slate-800 shrink-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={onClose}
              className="xl:hidden w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-800 transition-colors"
            >
          ...
          <div className="bg-slate-800 rounded-xl p-3">
            ...
            <div className="flex items-end gap-2">
              ...
              <span className="text-xs text-slate-500 mb-1">concluído</span>
            </div>
        ...
        {/* Module list */}
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-track-slate-900 scrollbar-thumb-slate-800">
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="relative w-full bg-black">
        {lesson.videoUrl ? (
          ...
          <div className="aspect-video flex flex-col items-center justify-center gap-4 text-slate-600 bg-slate-950">
        ...
        <div className="mb-5">
          ...
          {lesson.duration && (
            <div className="flex items-center gap-1.5 mt-2">
              ...
              <span className="text-xs text-slate-600">{formatDuration(lesson.duration)} de duração</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2.5 px-5 py-4 rounded-2xl font-black text-base transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed shadow-lg
              ${isCompleted
                ? 'bg-slate-800 border border-yellow-500/40 text-yellow-500 shadow-yellow-500/10'
          ...
          {lesson.materialUrl && (
            <a
              ...
              className="flex items-center justify-center gap-2 px-5 py-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 font-bold text-sm rounded-2xl transition-all active:scale-[0.98]"
            >
              <Download size={16} />
              📄 Baixar Material
            </a>
...
export const AcessoCurso: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
      ...
      {/* ── Topbar ── */}
      <header className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur border-b border-slate-800 shrink-0">
        {/* Nav row */}
        <div className="px-4 h-14 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0 flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              {activeLesson && (
                <p className="text-[10px] text-slate-500 truncate">{activeLesson.title}</p>
              )}
            </div>
          </div>

          {/* Aula atual / total */}
          <div className="hidden sm:flex items-center gap-1.5 bg-slate-800 px-3 py-1.5 rounded-xl">
          ...
          <button
            ...
            className="xl:hidden flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl transition-all"
          >
            <Menu size={15} className="text-slate-400" />
            <span className="text-xs text-slate-400 font-medium hidden sm:inline">Aulas</span>
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Player area */}
        <main className="flex-1 overflow-y-auto bg-slate-950 xl:bg-slate-900">
          {activeLesson ? (
            <VideoPlayer
              lesson={activeLesson}
